...
// @downloadURL  https://raw.githubusercontent.com/csmtc/web-scripts/main/cool18/dl_nov_all_cool18.js
...
*/
  ...
  function callback_dl(result, is_create_file = true, skip_header = false) {
    ...
    let main_node;
    if (isMobile) {
      title = pure_title(doc.querySelector("h1").textContent);
      main_node = doc.querySelector("#shownewsc");
    } else {
      title = pure_title(doc.querySelector(".show_content b").textContent);
      main_node = doc.querySelector("td.show_content>pre");
  ...
  if (window.location.href.search("wap.cool18.com") > 0) isMobile = true;
